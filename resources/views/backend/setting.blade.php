...
<div class="card ca-card">
    <h5 class="card-header ca-card-header">Edit Settings</h5>
    <div class="card-body ca-card-body">
    <form method="post" action="{{route('settings.update')}}" data-validate=true class="ca-form">
        @csrf 
        ...
        <div class="form-group ca-form-group ca-textfilled">
          <label for="timer" class="col-form-label">Question timer <span class="text-danger">*</span></label>
          <input type="text" class="form-control" name="timer" required value="{{$data->timer}}">
          @error('timer')
            <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
