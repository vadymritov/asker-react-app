...
<div class="card">
  ...
  <div class="card-body">
    <form method="post" action="{{route('payments.update',$payment->id)}}" data-validate="true">
      @csrf
      @method('PATCH')
      <div class="form-group">
        <label for="inputTitle" class="col-form-label">Payment Title <span class="text-danger">*</span></label>
        <input id="inputTitle" type="text" name="name" placeholder="Enter title"  value="{{$payment->name}}" class="form-control" required>
        @error('name')
      ...
      <div class="form-group">
          ...
          <input id="inputSlug" type="password" name="key" placeholder="Enter payment key"  value="{{!empty($payment->key)?decrypt($payment->key):""}}" class="form-control" minlength="5" required >
          @error('key')
          <span class="text-danger">{{$message}}</span>
          @enderror
      </div>
      <div class="form-group">
          ...
          <input id="inputUrl" type="password" name="secret" placeholder="Enter Payment Secret"  value="{{!empty($payment->secret)?decrypt($payment->secret):""}}" class="form-control" minlength="5" required >
          @error('secret')
          <span class="text-danger">{{$message}}</span>
          @enderror
      </div>
